            ...
            try:
                ...
                for j, img_path in enumerate(ordered_images, 1):
                    ...
                    if not segment_text or segment_text.upper() == "NULL":
                        consecutive_blank_pages += 1 # <--- NEW: Increment counter
                        log_and_print(f"      -> Blank page detected! ({consecutive_blank_pages}/2 consecutive)")
                        
                        if consecutive_blank_pages >= 2: # <--- NEW: Check if we reached 2
                            log_and_print(f"      -> 2 consecutive blank pages detected! Stopping transcription for remaining pages.")
                            break 
                        continue # Skip appending "NULL" to full_text, but move to next page
                        
                    # <--- NEW: If we reach here, the page has text. Reset counter to 0!
